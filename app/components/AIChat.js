...
const SUGGESTIONS = [
  'I feel lonely', 'I feel anxious', 'I feel sad',
  'I feel lost', 'I need guidance', 'I feel grateful',
  'I feel peaceful', 'I feel stressed', 'I feel hopeful',
]

const categoryLabel = {
  loneliness: 'Loneliness', anxiety: 'Anxiety', sadness: 'Sadness',
  financial_worry: 'Financial Worry', yearning: 'Yearning', grief: 'Grief',
  guilt: 'Guilt', anger: 'Anger', happiness: 'Happiness',
  gratitude: 'Gratitude', hope: 'Hope', contentment: 'Contentment',
  crisis: 'Crisis',
}
...
export default function AIChat() {
  ...
  useEffect(() => {
    const saved = (() => { try { return JSON.parse(localStorage.getItem('bayan_ai_messages') || '[]') } catch { return [] } })()
    if (saved.length > 0) setMessages(saved)
  }, [])

  useEffect(() => {
    localStorage.setItem('bayan_ai_messages', JSON.stringify(messages))
  }, [messages])
  ...
  useEffect(() => {
    if (!footnote) return
    const handler = (e) => {
      if (!e.target.closest('.fn-popup') && !e.target.closest('.verse-fn')) setFootnote(null)
    }
  ...
  const handleSubmit = async (text) => {
    ...
    try {
      const res = await fetch(`${API}/api/process-reflection`, {
        method: 'POST',
      ...
      if (data.status === 'rejected') {
        ...
        setMessages((prev) => [...prev, { role: 'bot', type: 'verses', category: data.category, language: data.language, verses: data.verses, timestamp: Date.now() }])
      } else if (data.status === 'conversation') {
        setMessages((prev) => [...prev, { role: 'bot', type: 'conversation', text: data.message, language: data.language, timestamp: Date.now() }])
      } else {
        setMessages((prev) => [...prev, { role: 'bot', type: 'rejected', text: 'Something went wrong. Please try again.', timestamp: Date.now() }])
      }
    } catch {
      setMessages((prev) => [...prev, { role: 'bot', type: 'rejected', text: 'Service is not running now. Try it later.', timestamp: Date.now() }])
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="view-content">
      <div className="ai-chat">
          ...
          {messages.length === 0 && !loading ? (
            <div className="empty-state">
              ...
              <div className="suggestions">
                {SUGGESTIONS.map((s) => (
                  <button key={s} className="suggestion-chip" onClick={() => handleSubmit(s)}>{s}</button>
                ))}
              </div>
            </div>
          ) : (
              ...
              <div key={i}>
                {msg.role === 'user' ? (
                  ...
                  <div className="message bot">
                    ...
                    <div className={`message-bubble${msg.language === 'arabic' ? ' arabic-text' : ''}`}>{msg.text}</div>
                    {msg.timestamp && <div className="message-time">{formatTime(msg.timestamp)}</div>}
                  </div>
                ) : msg.type === 'rejected' ? (
                  <div className="message bot">
                  ...
                  <div className="message bot">
                    <div className="message-label">Bayan &middot; {categoryLabel[msg.category] || msg.category}</div>
                    <div className="verse-card">
                      <div className="verse-card-header">
                        <div className="verse-card-icon">
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                        </div>
                        <span className="verse-card-title">{msg.language === 'arabic' ? 'الآيات' : 'Quranic Verses'}</span>
                        <span className="verse-card-subtitle">{categoryLabel[msg.category] || msg.category}</span>
                      </div>
                      <div className="verses-section">
                        {msg.verses.map((v, j) => {
                          const [surahNum, ayahNum] = v.reference.split(':')
                          const surah = SURAH_NAMES_AI[parseInt(surahNum)]
                          return (
                            <div key={j} className="verse-item">
                              <div className="verse-reference">
                                <span className={`verse-surah-name${msg.language === 'arabic' ? ' arabic-surah' : ''}`}>
                                  {msg.language === 'arabic' ? (surah?.ar || `سورة ${surahNum}`) : (surah?.en || `Surah ${surahNum}`)}
                                </span>
                                <span className="verse-ayah-num">{msg.language === 'arabic' ? `الآية ${ayahNum}` : `Verse ${ayahNum}`}</span>
                              </div>
                              <div className={`verse-text ${msg.language === 'arabic' ? 'arabic-text' : ''}`}>
                                {parseVerseText(v.text).map((part, pi) =>
                                  part.t === 'txt' ? <span key={pi}>{part.v}</span> : <sup key={pi} className="verse-fn" onClick={(e) => { e.stopPropagation(); setFootnote(part.id === footnote ? null : part.id) }}>{part.label}</sup>
                                )}
                              </div>
                            </div>
                          )
                      ...
                      {footnote && (
                        <div className="fn-popup">
                          ...
                          <div className="fn-popup-body">Reference ID: {footnote}</div>
                        </div>
                      )}
                    </div>
                    {msg.timestamp && <div className="message-time">{formatTime(msg.timestamp)}</div>}
        ...
        <div className="input-area">
          <form className="input-form" onSubmit={(e) => { e.preventDefault(); handleSubmit(input) }}>
